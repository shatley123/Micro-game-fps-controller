...
public class PlayerInputHandler : MonoBehaviour
{
    ...
    {
        return Input.GetAxis("Mouse X") * lookSensitivity * Time.deltaTime;
    }

    public float GetLookInputsVertical()
    {
        return -Input.GetAxis("Mouse Y") * lookSensitivity * Time.deltaTime;
    }
    ...
    //    if (CanProcessInput())
    //    {
    //        bool isGamepad = Input.GetAxis("Fire 1") != 0f;
    //        if (isGamepad)
    //        {
    ...
    public bool GetCrouchInputDown()
    {
        if (CanProcessInput())
        {
            return Input.GetButtonDown("Control");
        }
    ...
    float GetMouseOrStickLookAxis(string mouseInputName, string stickInputName)
    {
        if (CanProcessInput())
        {
            ...
            float i = isGamepad ? Input.GetAxis(stickInputName) : Input.GetAxisRaw("Horizontal");
